...
class Starling extends EventDispatcher
{
	...
	private static var sContextData = new Map<Stage3D, Dynamic>();
							 ...
							 renderMode:String="auto", profile:Dynamic="baselineConstrained")
	{
		...
		if (mStage3D.context3D && mStage3D.context3D.driverInfo != "Disposed")
		{
			...
			mShareContext = true;
			Lib.setTimeout(initialize, 1); // we don't call it right away, because Starling should
	...
	private function requestContext3D(stage3D:Stage3D, renderMode:String, profile:Dynamic):Void
	{
		...
		function requestNextProfile():Void
		{
			...
			try { StarlingUtils.execute(mStage3D.requestContext3D, renderMode, currentProfile); }
			catch (error:Error)
			{
				if (profiles.length != 0) Lib.setTimeout(requestNextProfile, 1);
				else throw error;
			}
		}
		...
		function onError(event:Event):Void
		{
			if (profiles.length != 0)
			{
				...
				Lib.setTimeout(requestNextProfile, 1);
			}
			else onFinished();
		}
	...
	public function get_backBufferWidth():Int { return mClippedViewPort.width; }

	/** Returns the current height of the back buffer. In most cases, this value is in pixels;
	 *  however, if the app is running on an HiDPI display with an activated
	 *  'supportHighResolutions' setting, you have to multiply with 'backBufferPixelsPerPoint'
	 *  for the actual pixel count.  */
	public function get_backBufferHeight():Int { return mClippedViewPort.height; }
	...
	public function set_simulateMultitouch(value:Bool):Void
	{
	...
	public function set_antiAliasing(value:Int):Void
	{
	...
	public function set_viewPort(value:Rectangle):Void { mViewPort = value.clone(); }
	 ...
	 *  memory analysis.</p>
	 */
	public function get_showStats():Bool { return mStatsDisplay && mStatsDisplay.parent; }
	public function set_showStats(value:Bool):Void
	{
	...
	public function get_nativeStage():flash.display.Stage { return mNativeStage; }
	...
	public function set_shareContext(value : Bool) : Void { mShareContext = value; }
	...
	public function set_supportHighResolutions(value:Bool):Void 
	{
	...
	public function set_touchProcessor(value:TouchProcessor):Void
	{
