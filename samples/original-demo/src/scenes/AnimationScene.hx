...
class AnimationScene extends Scene
{
	...
	public function new()
	{
		...
		mStartButton.name = 'startAnimation';
		mStartButton.addEventListener(Event.TRIGGERED, onStartButtonTriggered);
		mStartButton.x = Constants.CenterX - cast(mStartButton.width / 2);
		mStartButton.y = 20;
	...
	private function onStartButtonTriggered():Void
	{
		...
		tween.animate("rotation", StarlingUtils.deg2rad(90)); // conventional 'animate' call
